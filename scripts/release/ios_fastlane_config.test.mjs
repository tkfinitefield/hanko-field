...
test('rejects a production lane without manual signoff', async () => {
  ...
  const fastfilePath = join(rootDir, 'app/ios/fastlane/Fastfile');
  const fastfile = await readFile(fastfilePath, 'utf8');
  await writeFile(fastfilePath, fastfile.replace('    require_release_signoff("ios")\n', ''));
...
platform :ios do
  ...
  desc "Build the release IPA and upload it to App Store Connect after manual signoff."
  lane :production do
    require_release_signoff("ios")
    sh("cd \#{Shellwords.escape(APP_ROOT)} && flutter build ipa --release --export-options-plist=\#{Shellwords.escape(IOS_EXPORT_OPTIONS_PATH)}")
    upload_to_app_store(
      app_identifier: APP_IDENTIFIER,
      api_key_path: required_env("APP_STORE_CONNECT_API_KEY_PATH"),
